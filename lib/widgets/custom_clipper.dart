...
class WaveShapeClipper extends CustomClipper<Path> {
  ...
  getClip(Size size) {
    ...
    path.lineTo(size.width * 0.0, size.height * 0.7);

    var firstEndpoint = Offset(size.width * 0.6, size.height * 0.7);
    var firstControlPoint = Offset(size.width * 0.4, size.height * 0.6);

    path.quadraticBezierTo(firstControlPoint.dx, firstControlPoint.dy,
        firstEndpoint.dx, firstEndpoint.dy);

    var secondEndpoint = Offset(size.width, size.height * 0.8);
    var secondControlPoint = Offset(size.width * 0.8, size.height * 0.8);
...
class WaveShapeClipper1 extends CustomClipper<Path> {
  @override
  getClip(Size size) {
    final Path path = Path();
